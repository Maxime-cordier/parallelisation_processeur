...
InitClock;
	...
	__m128 vec_LE_MIN = _mm_set1_ps((float) - LE_MIN);
	__m128 vec_ETALEMENT = _mm_set1_ps((float) ETALEMENT);

ClockStart;

	for (i = 0 ; i < X*Y ; i=i+4) {
		__m128 vec_image = _mm_loadu_ps(&image[i]);

		__m128 vec_resultat = _mm_mul_ps(_mm_add_ps(vec_image, vec_LE_MIN), vec_ETALEMENT);

		_mm_storeu_ps(&resultat[i], vec_resultat);
	}
