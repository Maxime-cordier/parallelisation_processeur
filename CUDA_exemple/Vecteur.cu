...
/*	cudaSetDevice(1);	*/
	...
	long tmpGPU, copyCPUtoGPU, copyGPUtoCPU;
...
/*		printf("Resultat GPU %d     Resultat CPU %d \n",resultat[i], vecteur[i]+alpha);	*/
	...
	printf("Temps copie CPU -> GPU %ld \n", copyCPUtoGPU);
	printf("Temps calcul GPU %ld \n", tmpGPU);
	printf("Temps copie GPU -> CPU %ld \n", copyGPUtoCPU);
	printf("Vecteur %ld => Temps total GPU %ld \n", tailleVecteur, tmpGPU+copyCPUtoGPU+copyGPUtoCPU);
